...
def _conversation_read(
    ...
    # For buyer: unread messages are those sent by the seller
    # For seller: unread messages are those sent by the buyer
    if conversation.buyer_id == current_user_id:
        # Buyer: unread messages are those sent by the seller
        unread = sum(
            1
            for m in conversation.messages
            if not m.is_read and m.sender_id == conversation.seller_id
        )
    else:
        # Seller: unread messages are those sent by the buyer
        unread = sum(
            1
            for m in conversation.messages
            if not m.is_read and m.sender_id == conversation.buyer_id
        )
